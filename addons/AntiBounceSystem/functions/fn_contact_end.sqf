...
_angle = _vup vectorCos [0,0,1];
if (_angle < 0.8 && {!(_obj isKindOf "LAND")}) then {
	
	(boundingBoxReal _veh) params ["_c1", "_c2"];
	_c1 params ["_x1", "_y1"];
	_c2 params ["_x2", "_y2"];
	...
	_veh addForce [_veh vectorModelToWorld [0,0,-0.007*getMass _veh*(1-_angle)], _p];
};